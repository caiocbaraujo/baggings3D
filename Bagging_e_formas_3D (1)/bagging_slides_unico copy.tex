...
\graphicspath{{Figuras/}{../Figuras/}{nepo/img/}{../nepo/img/}}
...
\begin{frame}
    ...
    \begin{block}{Contexto}
        Técnicas de análise de agrupamentos classificam objetos em grupos distintos. O agrupamento de \textbf{formas geométricas} é relevante em medicina, robótica, engenharia civil e paleontologia.
    \end{block}

    \begin{alertblock}{Desafio central}
        Formas 3D vivem em \textbf{espaços não euclidianos} (espaço estratificado com singularidades). Não basta usar distância euclidiana e média aritmética usuais.
    \end{alertblock}

    \begin{itemize}
        \item É preciso remover efeitos de \textbf{localização}, \textbf{escala} e \textbf{rotação}
        \item Comparar objetos com distâncias adequadas ao espaço de formas
    \end{itemize}
\end{frame}

\begin{frame}
    \frametitle{Gap na Literatura e Objetivo}
    \framesubtitle{Contribuição do artigo}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Gap}
            \begin{itemize}
                ...
                \item Passar de 2D para 3D não é ``só mais uma dimensão''
...
\begin{frame}[shrink=8]
    \frametitle{Seção 2: Fundamentos}
...
\begin{frame}[shrink=10]
    ...
    \begin{block}{Pré-forma (eq.~(1) do artigo)}
        \begin{equation}
            Z = \frac{HX}{\|HX\|}, \qquad
...
\begin{frame}[shrink=8]
    ...
    \vspace{0.2em}
    \footnotesize\textit{Nota:} em 3D o espaço $\Sigma_m^k$ não é variedade suave; na prática assume-se variação restrita, longe das singularidades.
\end{frame}
...
\begin{frame}
    \frametitle{Seção 3: Agrupamento de formas 3D}
    ...
    \begin{itemize}
        ...
        \item Métodos: k-means (Vinué et al., 2014), CLARANS e Hill Climbing (adaptações deste artigo)
    \end{itemize}
...
\begin{frame}[shrink=8]
    ...
    \begin{block}{Critério WSS --- equação (4)}
        \begin{equation}
...
\begin{frame}[shrink=5]
    ...
    Parâmetros: \texttt{numlocal} (buscas locais), \texttt{maxneighbor} (máx.\ vizinhos examinados). Adaptação 3D: Algoritmo~3 do artigo (distâncias riemannianas).
\end{frame}
...
\begin{frame}[shrink=8]
    ...
    \begin{block}{Dissimilaridade total --- equação (5)}
        \begin{equation}
    ...
    \textbf{Algoritmo 4}: atualização dos medoides nos passos de reatribuição. Pode ficar em ótimo local; o Bagging (Seção~3.4) visa reduzir essa instabilidade.
\end{frame}

\section[Bagging]{A Proposta: Bagging}

\begin{frame}
    \frametitle{O que é Bagging?}
    ...
    \begin{itemize}
        \item No artigo: procedimento \textbf{BagClust1} (Dudoit \& Fridlyand, 2003)
...
\begin{frame}
    ...
    \framesubtitle{Passo a passo (BagClust1)}
...
\begin{frame}[shrink=5]
    \frametitle{Seção 4: Avaliação numérica}
    ...
    \begin{block}{Parâmetros dos algoritmos (artigo)}
        \footnotesize
        \begin{itemize}
...
\begin{frame}[shrink=12]
    ...
    \framesubtitle{Isotropia e anisotropia (Seção~4.1)}
...
\begin{frame}[shrink=8]
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Métricas e inferência}
            \begin{itemize}
                ...
                \item Tabelas 1--3 (isotropia) e 4--6 (anisotropia): média $\bar{x}$ e desvio-padrão \textit{sd} sobre 50 réplicas
    ...
    \vspace{0.2em}
    \footnotesize\textit{Nota:} dados reais (crânios, cérebros) usam uma única execução e \textbf{ganho relativo} em RI/FMI; simulação usa distribuição das 50 réplicas.
\end{frame}
...
\begin{frame}[shrink=8]
    \frametitle{O Bagging Funciona?}
    \framesubtitle{Mensagens principais}
...
\begin{frame}[shrink=10]
    \frametitle{Dataset 1: Crânios de Macacos}
    ...
    \vspace{0.4cm}
    \centering
    \textbf{Tabela 7:} Resultados para o conjunto de dados Crânio de Macaco (\textit{Macaque Skull}).\\[0.1cm]
...
\begin{frame}[shrink=10]
    \frametitle{Dataset 2: Cérebros Humanos}
    ...
    \vspace{0.4cm}
    \centering
    \textbf{Tabela 8:} Resultados para o conjunto de dados Cérebros (\textit{Brains}).\\[0.1cm]
    \resizebox{0.95\linewidth}{!}{
        \begin{tabular}{@{}lcccccc@{}}
        \toprule
        Methods & \multicolumn{2}{c}{Rand index} & RI relative & \multicolumn{2}{c}{Fowlkes-mallows index} & FMI relative \\
...
\begin{frame}
    ...
    \framesubtitle{Benefício claro do Bagging em dados mais complexos}
    \centering
...
\begin{frame}
    ...
    \begin{block}{Principal mensagem}
    ...
    \begin{itemize}
        ...
        \item \textbf{k-means}: melhorias limitadas na simulação; ganhos claros no dataset Brains
