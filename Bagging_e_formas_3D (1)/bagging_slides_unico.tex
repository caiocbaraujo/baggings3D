...
\begin{frame}[t]
    ...
    \framesubtitle{Representação geométrica de objetos (Kendall, 1984; Dryden \& Mardia, 2016)}

    \begin{columns}[T,onlytextwidth]
        \column{0.58\textwidth}
        \footnotesize
        Ramo da estatística que trabalha com a \textbf{geometria} dos objetos, representados por \textbf{marcos} (\textit{landmarks}) --- matriz $X \in \mathbb{R}^{k\times m}$.
        \begin{itemize}
            \setlength{\itemsep}{0.2em}
            \item \textbf{Forma}: informação que resta após remover \textbf{posição}, \textbf{escala} e \textbf{rotação} (Kendall, 1977)
            \item Comparação exige métricas próprias (Procrustes / riemanniana), não a distância euclidiana usual
            \item Conceitos fundamentais: Bookstein et al.\ (1986); Kendall (1984)
        \end{itemize}
        \column{0.40\textwidth}
        \figpainel[0.32\textheight]{cerebro.png}{Ex.: morfologia do córtex cerebral em estudos clínicos (Brignell et al., 2010).}
    \end{columns}
\end{frame}
