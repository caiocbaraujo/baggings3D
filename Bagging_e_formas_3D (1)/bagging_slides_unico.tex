...
\begin{frame}[t]
    \frametitle{Análise estatística de formas}
    \framesubtitle{\textit{Statistical Shape Analysis} (Nascimento et al., 2024)}

    \vspace{-0.1cm}
    \begin{columns}[T,onlytextwidth]
        \column{0.57\textwidth}
        \begin{block}{Um ramo da estatística}
            \small
            A \textbf{análise estatística de formas} (\textit{Statistical Shape Analysis}) trabalha com \textbf{representações geométricas} e \textbf{formas de objetos}: a \textbf{geometria} é a informação relevante, após remover efeitos de posição, escala e rotação (Dryden \& Mardia, 2016).
        \end{block}
        \vspace{0.15em}
        \small
        Representações geométricas são centrais em estudos em diversas áreas; o agrupamento de \textbf{formas 3D} tornou-se crucial para analisar e interpretar dados tridimensionais.
        \column{0.41\textwidth}
        \figpainel[0.30\textheight]{fóssil.png}{Forma como informação morfológica, independente de escala e posição.}
    \end{columns}
\end{frame}

\begin{frame}[shrink=8]
    \frametitle{Análise de agrupamentos}
    \framesubtitle{Métodos particionais (Everitt et al., 2011)}

    \begin{block}{Ideia central}
        \small
        Formar grupos com \textbf{homogeneidade interna} e \textbf{heterogeneidade externa}: objetos semelhantes no mesmo grupo e grupos distintos entre si.
    \end{block}

    \begin{itemize}
        \footnotesize
        \item Dois grandes tipos: \textbf{hierárquicos} e \textbf{particionais} (foco deste trabalho)
        \item Métodos particionais partem de uma partição inicial com $K$ grupos pré-definidos e reatribuem objetos até convergir (Friedman \& Rubin, 1967)
    \end{itemize}

    \begin{block}{Os três algoritmos adaptados pelo artigo}
        \footnotesize
        \begin{itemize}
            \item \textbf{K-means}: minimiza a soma de quadrados intragrupos (WSS); usa centroides
            \item \textbf{CLARANS}: usa medoides (objetos reais); mais robusto a \textit{outliers} (Ng \& Han, 2002)
            \item \textbf{Hill Climbing}: busca local que otimiza um critério de dissimilaridade (Friedman \& Rubin, 1967)
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}[t]
    ...
    \framesubtitle{Medicina, biologia, engenharia e arqueologia (Adams \& Otárola-Castillo, 2013)}

    \vspace{-0.15cm}
    \begin{block}{Onde a geometria importa}
        \footnotesize
        \begin{itemize}
            \setlength{\itemsep}{0.12em}
            \item \textbf{Medicina}: morfologia de órgãos (cérebro, coração); neurociência e diagnóstico (Brignell et al., 2010)
            \item \textbf{Biologia}: classificação de espécies pela morfologia de estruturas anatômicas
            \item \textbf{Engenharia}: controle de qualidade e análise de peças manufacturadas em 3D
            \item \textbf{Arqueologia}: comparação de artefatos e morfologia de fósseis entre grupos ou culturas
        \end{itemize}
    \end{block}
    \vspace{0.15em}
    \begin{columns}[T,onlytextwidth]
        \column{0.24\textwidth}
        \figpainel[0.14\textheight]{cerebro.png}{Medicina}
        \column{0.24\textwidth}
        \figpainel[0.14\textheight]{abelhas.png}{Biologia}
        \column{0.24\textwidth}
        \figpainel[0.14\textheight]{motor.png}{Engenharia}
        \column{0.24\textwidth}
        \figpainel[0.14\textheight]{fóssil.png}{Arqueologia}
    \end{columns}
\end{frame}

\begin{frame}[t]
    \frametitle{Representação geométrica e o problema do agrupamento}
    \framesubtitle{Da forma ao agrupamento em 3D (Kendall, 1984; Bookstein et al., 1986)}

    \begin{columns}[T,onlytextwidth]
        \column{0.58\textwidth}
        \footnotesize
        Na abordagem geométrica da SSA, utiliza-se a \textbf{representação do próprio objeto} --- por exemplo, \textbf{marcos} (\textit{landmarks}) em $\mathbb{R}^{k\times m}$, com $m=3$.
        \begin{itemize}
            \setlength{\itemsep}{0.18em}
            \item \textbf{Forma}: informação que permanece ao remover \textbf{posição}, \textbf{escala} e \textbf{rotação} (Kendall, 1977)
            \item Vários trabalhos adaptaram algoritmos de \textbf{agrupamento} a esse contexto (Amaral et al., 2010; Vinué et al., 2014)
            \item Objetivo deste artigo: melhorar o agrupamento de \textbf{formas 3D} com \textbf{Bagging}
        \end{itemize}
        \column{0.40\textwidth}
        \figpainel[0.32\textheight]{coração.png}{Ex.: morfologia cardíaca --- forma, não apenas tamanho.}
    \end{columns}
\end{frame}

\begin{frame}[shrink=5]
    \frametitle{Revisão: do agrupamento de formas 2D ao 3D}
    \framesubtitle{Posicionamento na literatura}
    ...
    \begin{table}
        ...
        \begin{tabular}{@{}llp{5.6cm}@{}}
            \toprule
            \textbf{Trabalho} & \textbf{Dim.} & \textbf{Contribuição} \\
            \midrule
            Amaral et al.\ (2010) & 2D & K-means adaptado a formas (oceanografia) \\
            Assis et al.\ (2021) & 2D & \textbf{Bagging + K-means} para formas planas \\
            Vinué et al.\ (2014) & 3D & K-means e K-means podado para formas 3D \\
            Nascimento et al.\ (2024) & 3D & K-means, \textbf{CLARANS}, \textbf{Hill Climbing} + \textbf{Bagging} \\
            \bottomrule
        \end{tabular}
    \end{table}
    ...
    \begin{itemize}
        \footnotesize
        \item Em \textbf{2D}, o espaço de formas é variedade riemanniana (mais tratável)
        \item Em \textbf{3D}, é espaço \textbf{estratificado com singularidades} (Bhattacharya \& Bhattacharya, 2012)
        \item Lacuna: CLARANS e Hill Climbing ainda não haviam sido adaptados a formas 3D
    \end{itemize}
\end{frame}
...
\begin{frame}[t]
    \frametitle{Bagging em agrupamento: antecedentes}
    \framesubtitle{Por que combinar Bagging com clustering}

    \begin{itemize}
        \footnotesize
        \setlength{\itemsep}{0.25em}
        \item \textbf{Bagging} (Breiman, 1996): \textit{ensemble} baseado em amostras \textit{bootstrap}
        \item \textbf{Leisch (1999)}: \textit{bagged clustering} --- combina métodos hierárquicos e particionais
        \item \textbf{Dudoit \& Fridlyand (2003)}: procedimento \textbf{BagClust1} estabiliza métodos baseados em medoides (PAM)
        \item \textbf{Assis et al.\ (2021)}: Bagging melhora o K-means em formas \textbf{2D}
    \end{itemize}

    \begin{alertblock}{Motivação para este artigo}
        \footnotesize
        Métodos particionais são \textbf{instáveis} (inicialização aleatória, \textit{outliers}, ótimos locais). O Bagging busca maior estabilidade --- recurso ainda não explorado em formas \textbf{3D}.
    \end{alertblock}
\end{frame}

\begin{frame}[t]
    \frametitle{Objetivos e contribuição}
    \framesubtitle{Contribuição de Nascimento et al.\ (2024)}

    \vspace{-0.1cm}
    \begin{block}{Lacuna específica}
        \footnotesize
        Vinué et al.\ (2014) adaptaram apenas o K-means a formas 3D; \textbf{CLARANS}, \textbf{Hill Climbing} e o uso de \textbf{Bagging} permaneciam inexplorados nesse contexto.
    \end{block}
    \vspace{0.15em}
...
\begin{frame}[allowframebreaks]
    ...
    \begin{thebibliography}{99}
        ...
        \textit{Cluster Analysis}, 5th ed.
        Wiley, 2011.

        \bibitem{amaral2010}
        Amaral, G.\ J.\ A.; Dore, L.\ H.; Lessa, R.\ P.; Stosic, B.
        k-means algorithm in statistical shape analysis.
        \textit{Communications in Statistics --- Simulation and Computation}, 39(5):1016--1026, 2010.

        \bibitem{leisch1999}
        Leisch, F.
        Bagged clustering.
        \textit{Working Paper 51}, SFB Adaptive Information Systems and Modelling in Economics and Management Science, 1999.

        \bibitem{bhattacharya2012}
        Bhattacharya, A.; Bhattacharya, R.
        \textit{Nonparametric Inference on Manifolds: With Applications to Shape Spaces}.
        Cambridge University Press, 2012.

        \bibitem{friedman1967}
        Friedman, H.\ P.; Rubin, J.
        On some invariant criteria for grouping data.
        \textit{Journal of the American Statistical Association}, 62(320):1159--1178, 1967.

        \bibitem{bookstein1986}
        Bookstein, F.\ L.
        Size and shape spaces for landmark data in two dimensions.
        \textit{Statistical Science}, 1(2):181--222, 1986.

        \bibitem{adams2013}
        Adams, D.\ C.; Otárola-Castillo, E.
        Geomorph: an R package for the collection and analysis of geometric morphometric shape data.
        \textit{Methods in Ecology and Evolution}, 4(4):393--399, 2013.

        \bibitem{brignell2010}
        Brignell, C.\ J.; Dryden, I.\ L.; Gattone, S.\ A.; Park, B.; Leask, S.; Browne, W.\ J.; Flynn, S.
        Surface shape analysis with an application to brain surface asymmetry in schizophrenia.
        \textit{Biostatistics}, 11(4):609--630, 2010.
    \end{thebibliography}
\end{frame}
